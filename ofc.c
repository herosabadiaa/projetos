...
// Some Defines
//----------------------------------------------------------------------------------
#define MAX_DECK 60 //Define o maximo de cartas num deck
...
int main(void){
    ...
    while(!WindowShouldClose()){
        ...
        if (arrasto.id != 0){ //Altera a posicao de uma carta que esta sendo arrastada
            arrasto.cordenada = (Vector2){mousePosition.x-20, mousePosition.y-20};
        }
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)){ //Verifica Click LMB(Left Mouse Button)
            ...
            for (int i=0;i<MAX_HAND; i++){ //Verfica a colisao do mouse com as posicoes da mao
                if (CheckCollisionPointRec(GetMousePosition(), mao[i].area)){
            ...
            if (arrasto.id != 0){
                for(int i = 0; i<MAX_POSITION; i++){
                    if((CheckCollisionPointRec(GetMousePosition(), campos[i].area))){
                        if(campos[i].vazia == true){
                            campos[i].carregada = arrasto;
                            arrasto = (Carta){0};
                            campos[i].vazia = false;
                            break;
                        }
                    }
                }
        ...
        BeginDrawing();
            ...
            char deckText[64]; //Desenha o texto de quantas cartas disponiveis
            ...
            for(int i=0;i<MAX_POSITION;i++){
                ...
                if(campos[i].vazia == false){ //Desenha as cartas caso haja alguma
                    DrawTexture(campos[i].carregada.textura, campos[i].area.x, campos[i].area.y, WHITE); 
                }
            }
